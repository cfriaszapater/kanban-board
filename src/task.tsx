...
import { beginTaskEditing, finishTaskEditing } from "./store/cards/actions";
...
const ContainerTextArea = styled.textarea<ContainerProps>`
  border: 1px solid lightgrey;
  border-radius: 2px;
  padding: 8px;
  margin-bottom: 8px;
...
export class TaskView extends React.Component<TaskProps> {
  ...
  handleBlur = (event: React.FocusEvent<HTMLTextAreaElement>) => {
    console.log("blur with value ", event.target.value);
    this.props.dispatch(finishTaskEditing(this.props.task, event.target.value));
  };
    ...
    return (
      <Draggable draggableId={this.props.task.id} index={this.props.index}>
        {(provided, snapshot) => {
          ...
          if (
            typeof this.props.task.editing === "undefined" ||
